 ...
 */
Camera::Camera(const char* dev_name, struct camera_format* conf, const char* format_string,
               const unsigned char disable_libv4l2)
    : dev_name(dev_name), disable_libv4l2(disable_libv4l2)
{
    ...
    if (format_string != 0) {
        ...
        set_format(config.width, config.height, config.pixformat, config.rate_numerator,
...
bool Camera::get_capability()
{
    ...
    DBG_PRINTF("kernel version: %u.%u.%u", (v4l2_s.capability.version >> 16) & 0xFF,
               (v4l2_s.capability.version >> 8) & 0xFF, v4l2_s.capability.version & 0xFF);
    ...
    DBG_PRINTF("V4L2_CAP_VIDEO_CAPTURE : %s",
               (v4l2_s.capability.capabilities & V4L2_CAP_VIDEO_CAPTURE) ? "True" : "False");
...
bool Camera::set_buffer()
{
    ...
    for (unsigned int i = 0; i < buffer_count; i++) {
        ...
        buffers[i].buffer = (unsigned char*)mmap(NULL, v4l2_s.buffer.length, PROT_READ | PROT_WRITE,
...
bool Camera::enumerate_image_formats(const enum v4l2_buf_type buf_type)
{
    ...
    while (xioctl(VIDIOC_ENUM_FMT, &v4l2_s.fmtdesc) != -1) {
        ...
        DBG_PRINTF("Format pixel format: %c, %c, %c, %c", (v4l2_s.fmtdesc.pixelformat >> 0) & 0xFF,
    ...
    while (xioctl(VIDIOC_ENUM_FRAMESIZES, &v4l2_s.frmsizeenum) != -1) {
        ...
        switch (v4l2_s.frmsizeenum.type) {
            case V4L2_FRMSIZE_TYPE_DISCRETE:
                DBG_PRINTF("Discrete :: Width: %d, Height: %d", v4l2_s.frmsizeenum.discrete.width,
                           v4l2_s.frmsizeenum.discrete.height);

                desc.clear();
                desc = description +
                       (" " + Withrobot::to_string<int>(v4l2_s.frmsizeenum.discrete.width) + " x " +
            ...
            case V4L2_FRMSIZE_TYPE_STEPWISE:
                DBG_PRINTF(
                    "Stepwise :: Width step(max/min): %d (%d / %d), Height step(max / min): %d (%d "
                    "/ %d)",
                    v4l2_s.frmsizeenum.stepwise.step_width, v4l2_s.frmsizeenum.stepwise.max_width,
                    v4l2_s.frmsizeenum.stepwise.min_width, v4l2_s.frmsizeenum.stepwise.step_height,
                    v4l2_s.frmsizeenum.stepwise.max_height, v4l2_s.frmsizeenum.stepwise.min_height);
                break;
 ...
 * @return
 */
bool Camera::enumerate_frame_intervals(const unsigned int pixelformat, const unsigned int width,
                                       const unsigned int height, std::string& description)
{
...
int Camera::enumerate_controls()
{
    ...
    while (query_ioctl(VIDIOC_QUERYCTRL, &v4l2_s.queryctrl) == 0) {
        ...
        DBG_PRINTF("id: 0x%X, %s, flags: %d", v4l2_s.queryctrl.id, v4l2_s.queryctrl.name,
                   v4l2_s.queryctrl.flags);
...
int Camera::enumerate_control_menu()
{
    ...
    for (v4l2_s.querymenu.index = v4l2_s.queryctrl.minimum;
         (int)v4l2_s.querymenu.index <= v4l2_s.queryctrl.maximum; v4l2_s.querymenu.index++) {
        if (xioctl(VIDIOC_QUERYMENU, &v4l2_s.querymenu) == 0) {
            DBG_PRINTF("id: %s, 0x%X, %s", v4l2_s.queryctrl.name, v4l2_s.querymenu.index,
                       v4l2_s.querymenu.name);
        }
        else {
...
int Camera::valid_controls(std::vector<std::pair<const char*, unsigned int> >& list)
{
    ...
    try {
        for (it = valid_control_list.begin(); it != valid_control_list.end(); ++it) {
            list.push_back(
                std::pair<const char*, unsigned int>(it->first.c_str(), it->second.type));
        }
    }
    catch (...) {
...
bool Camera::get_current_format()
{
    ...
    DBG_PRINTF(
        "V4L2_CAP_TIMEPERFRAME : %s",
        (v4l2_s.streamparm.parm.capture.capability & V4L2_CAP_TIMEPERFRAME) ? "True" : "False");
 ...
 * @return
 */
bool Camera::set_format(unsigned int width, unsigned int height, unsigned int pixelformat,
                        unsigned int rate_numerator, unsigned int rate_denomonator)
{
    get_current_format();
...
bool Camera::set_format(const char* format_description)
{
    ...
    set_format(v4l2_s.frmivalenum.width, v4l2_s.frmivalenum.height, v4l2_s.frmivalenum.pixel_format,
               v4l2_s.frmivalenum.discrete.numerator, v4l2_s.frmivalenum.discrete.denominator);
...
int Camera::get_buffer(unsigned char* buffer, const unsigned int size)
{
    ...
    if (v4l2_s.buffer.bytesused == size ||
        ...
        DBG_PRINTF("Warning :: Different buffer size. v4l2: %d, User: %d", v4l2_s.buffer.bytesused,
                   size);
    }
...
int Camera::xioctl(int IOCTL_X, void* arg)
{
    ...
    if (ret && (tries <= 0)) {
        DBG_PRINTF("V4L2_CORE: ioctl (%i) retried %i times - giving up: %s)", IOCTL_X,
                   WITHROBOT_CAMERA_IOCTL_RETRY, strerror(errno));
    }
